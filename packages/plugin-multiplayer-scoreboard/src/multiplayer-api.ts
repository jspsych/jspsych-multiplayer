 ...
 * writes this client's score once (`update`, which merges the score key into this client's slot
 * without disturbing what earlier trials pushed there), then waits (`wait`) until the group is ready
 * — kept as two separate calls so a write failure is distinguishable from a barrier timeout — then
 * computes and renders the final board. It declares `update`/`getAll`/`wait` plus `participantId` —
 * but not `subscribe`; the live standings demo (examples/live-scoreboard-room.html) renders from the
 * pure core plus `jsPsych.multiplayer.subscribe` directly, without going through this interface.
 *
