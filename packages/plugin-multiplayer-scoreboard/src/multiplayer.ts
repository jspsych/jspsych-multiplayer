...
export function getMultiplayer(jsPsych: JsPsych): Multiplayer {
  ...
  if (!multiplayer) {
    throw new Error(
      "plugin-multiplayer-scoreboard: this plugin needs a version of jsPsych with the multiplayer API " +
...
/**
 * The default key for this participant's next scoreboard gate: `scoreboard-1`,
 * `scoreboard-2`, and so on. Every participant passes gates in the same order,
