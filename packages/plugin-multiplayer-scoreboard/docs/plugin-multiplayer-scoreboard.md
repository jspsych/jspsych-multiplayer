...
| `label`          | string            | `null`                                        | Display name this client writes for its own row (dynamic). Defaults to the participantId.                                                                                                                                                  |
| `group_size`     | integer           | `null`                                        | Wait until **at least** this many participants have reported before revealing the board. `null` counts the members of a sealed group who haven't left; without a sealed group it reveals immediately (trusts an upstream barrier).                                                                                          |
| `timeout`        | integer           | `30000`                                       | Milliseconds to wait for `group_size` reporters. On expiry the board still renders, with `multiplayer_outcome: "timeout"`. `null`, `0`, or a negative value waits forever.                                                                                                       |
...
| `participants`   | array             | `null`                                        | Participants the board depends on; if one leaves first, the board is shown with `multiplayer_outcome: "participant_left"`. `null` means the other group members who haven't left; `[]` ignores departures.          |
...
| `display_label`  | function          | `null`                                        | `(id, group) => string` mapping any participantId to the displayed name, overriding written labels. `group` is the session's shared data.                                                                                                                                         |
...
| `multiplayer_outcome` | string | How waiting ended: `"completed"`, `"timeout"`, `"participant_left"`, or `"connection_lost"` (board may be partial on any but `"completed"`); `null` on another failure (see `error`). |
| `left_participant` | string | The ID of the participant who left, when `multiplayer_outcome` is `"participant_left"`; `null` otherwise. |
| `error`       | string  | Any other failure while reporting to the group (e.g. an adapter error); `null` otherwise.           |
