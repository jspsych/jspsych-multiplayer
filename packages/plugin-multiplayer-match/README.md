...
| `participants`     | array \| null | `null`                         | Participants the match depends on. If one leaves the session before the group is ready, the trial ends unmatched with `partner_left: true`. `null` means every other participant who hasn't already left when this participant arrives; `[]` ignores departures.                                                                                            |
