...
| `push_data`        | object      | `{}`                           | Extra data this client contributes into the shared session (merged alongside `joinedAt`). Must be JSON-safe: the session deep-copies it with `JSON.stringify`, so a `Date` arrives as a string and `undefined`/`Map`/`Set`/`NaN` values do not survive. |
