 ...
 *
 * Like `plugin-multiplayer-role`, matching is a barrier-then-compute trial: it merges this client's
 * `joinedAt`/round data into its slot (`update`), then `wait`s until the group is ready, then
 * partitions the resolved snapshot. It declares `participantId`/`update`/`getAll`/`get`/`wait` —
 * `get` because `joinedAt` is first-seen, so this client's existing slot has to be read before the
 * write. Mock-based tests implement this same interface, so the wrapper is exercised end-to-end with
 * no live group session.
 */
