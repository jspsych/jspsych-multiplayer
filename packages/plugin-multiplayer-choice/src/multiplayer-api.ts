 ...
 * collects this participant's pick, pushes it once (`push`), then waits (`wait`) until the group has
 * chosen. It additionally declares `participantId` and `get`, needed to read this client's own slot
 * and push its choice back without clobbering other keys (`push` REPLACES the whole slot).
 * `subscribe`/`communicate` are not declared: the reveal renders once from the barrier snapshot.
 *
