 ...
 * primitive — plus `participantId` and `get`, which seed this client's own message array once at
 * trial start; every send then goes through `update`, which merges only the chat key into the slot
 * (`push` REPLACES the whole slot; see `chat-core.ts`). `wait` is not declared: the end condition is
 * evaluated inside the single subscription callback, so this plugin has no wait that a cancellation
