 ...
 * The thin `index.ts` trial wires these functions to `subscribe`/`update` and the DOM.
 *
 * ## Data model
 * Each participant writes only their own data, so chat history is modeled as an **append-only
 * array each participant keeps under one key** (`chat_messages`, in the trial's own scope). The
 * rendered transcript is the merge of every participant's array, sorted and de-duplicated.
 */
