 ...
 *
 * Like `plugin-multiplayer-chat`, the turn coordinator is a continuously-open trial: it declares the
 * real-time **`subscribe`** primitive plus `participantId` and `get` (to read/append this client's own
 * slot without clobbering other keys — `push` REPLACES the whole slot). `wait` is not declared: the
 * start gate and the "whose turn" logic are evaluated inside the single subscription callback.
 *
