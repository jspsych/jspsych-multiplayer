...
| `participants` | array \| null           | `null`                                | Participants the barrier depends on. If one of them leaves the session first, the trial ends with `partner_left: true`. `null` means every other participant who is connected when the wait starts; `[]` ignores departures. |
