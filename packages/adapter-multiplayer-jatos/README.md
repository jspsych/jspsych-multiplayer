...
Once connected, multiplayer plugins and `jsPsych.multiplayer` (`update`, `replace`, `get`, `getAll`, `presence`, `group`, `subscribe`, `wait`, `sealGroup`, `waitForGroup`) work against the JATOS group session.
...
How long to wait for the group channel to open, and whether to give up on a channel that stays down, are options of `jsPsych.multiplayer.connect()`:

```js
await jsPsych.multiplayer.connect(new JatosAdapter(), {
  connectTimeout: 10000, // default 20000
  reconnectTimeout: 5 * 60_000, // default: never give up
});
```

### Upgrading from 0.1

The adapter's `connectTimeoutMs` and `closeAfterReconnectingMs` options were removed; the adapter warns and ignores them. Pass jsPsych's own options instead:

Removed | Use instead
--------|------------
`new JatosAdapter({ connectTimeoutMs: n })` | `jsPsych.multiplayer.connect(adapter, { connectTimeout: n })`
`new JatosAdapter({ closeAfterReconnectingMs: n })` | `jsPsych.multiplayer.connect(adapter, { reconnectTimeout: n })`

`jsPsych.multiplayer.sealGroup()` now fails with an `unsupported` error, before contacting JATOS, when the loaded jatos.js has no `jatos.setGroupFixed()`.
...
- **Groups.** JATOS assigns each arriving participant to a group with room, on its server, and `connect()` resolves once it has. `group()` reports the batch's `maxActiveMembers` as the size and `jatos.groupMembers` as the members. With `sealWhenFull`, every member asks JATOS to fix the group once it is full (the server treats repeats as no-ops, and a failed request is retried on the next change); `sealGroup()` fixes it on request. The group counts as sealed only once JATOS confirms. Hold participants until then with `jsPsych.multiplayer.waitForGroup()`.
- **Telling the group it is sealed.** JATOS confirms a fix only to the member who asked, and `jatos.groupMembers` drops members who leave even after the group is fixed. So the member whose fix is confirmed writes the final roster, sorted, into the group session under the reserved key `$sealed` (with its own ID as `by`), retrying with backoff if the write fails. Every member that reads a record it trusts reports `sealed: true` with that roster, and stops asking JATOS to fix the group. Rosters are merged, never shrunk, so a member who drops out after the seal stays on everyone's roster. `getAll()` leaves out `$sealed` and any other key that starts with `$`; participant IDs can't contain `$`.
- **Which seal records count.** JATOS lets any member write any group session key, so a member checks a record before believing it: the writer (`by`) and this participant must be on its roster, everyone `jatos.groupMembers` lists right now must be on it too (after a real fix nobody new can join, so the current members are always part of the final roster), and everyone on it must have been seen as a member or have written data. A record that fails these checks is ignored with a console warning. The checks catch stale, buggy, or mistaken records; they can't stop a participant who deliberately forges one, since JATOS doesn't authenticate group session writes (the same participant could also overwrite others' data).
...
- **Connection status.** When this participant's group channel drops, jatos.js reopens it by itself. The adapter reports the connection as `reconnecting` while it is down and `connected` once it reopens, and waits as long as it takes; jsPsych's `reconnectTimeout` option decides when to give up and end the session. While the channel is down, jatos.js clears its copy of the group session, so the adapter keeps serving the last data it read.
- **Rejoining.** Every connection on a page uses the same study result id, and the adapter reports every drop and reopen. So a participant whose channel reopens on the same page within the others' `dropoutTimeout` is `connected` again for them; `left` is final. A participant who reloads the study page restarted their experiment: other participants count them as `left`, and the reloaded page finds `jsPsych.multiplayer.restarted === true`. See [Handling dropouts](https://multiplayer.jspsych.org/guides/handling-dropouts) in the docs.
- **Writes.** The JATOS group session uses optimistic concurrency, so simultaneous writes can hit version conflicts. `push()` makes up to 3 quick attempts with backoff and jitter, then rejects with the underlying error as `cause`; jsPsych retries a failed push with its own backoff. Each attempt re-sends the same `participantId → data` write, so a retry can never lose or double-apply another participant's update. A push made while the channel is down rejects at once, so it never holds up later writes; jsPsych sends the latest data again once the channel reopens. `getAll()` returns each participant's data exactly as it was pushed.
- **Connecting.** `connect()` resolves when the group channel opens. It rejects if joining fails, if jatos.js refuses to open a channel, or when jsPsych aborts the attempt (its `connectTimeout`, default 20 s). jatos.js refuses to open a channel while the previous one's socket is still closing (for example just after a `disconnect()`); the adapter retries that case until the socket has closed or the attempt is aborted. If the connection attempt is cancelled and the channel opens anyway, the adapter leaves the group.
