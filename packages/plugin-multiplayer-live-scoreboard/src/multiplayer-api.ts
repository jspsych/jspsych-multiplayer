 ...
 *
 * Unlike the barrier-based end-of-game `plugin-multiplayer-scoreboard` (which uses `push`/`getAll`/
 * `wait`), the *live* scoreboard is a continuously-open trial that re-renders as peers report, so it
 * additionally declares **`subscribe`** — the real-time primitive — plus `participantId` and `get`,
 * needed to read this client's own slot and push its score back without clobbering other keys (`push`
 * REPLACES the whole slot). `wait`/`communicate` are not declared: the end condition is evaluated
 * inside the subscription callback, not against a barrier.
 *
