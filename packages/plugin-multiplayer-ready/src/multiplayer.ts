...
export function getMultiplayer(jsPsych: JsPsych): Multiplayer {
  ...
  if (!multiplayer) {
    throw new Error(
      "multiplayer-ready: this plugin needs a version of jsPsych with the multiplayer API " +
...
/**
 * The default key for this participant's next ready gate: `ready-1`,
 * `ready-2`, and so on. Every participant passes gates in the same order,
