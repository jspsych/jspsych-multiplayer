 ...
 *
 * Near-identical surface to `plugin-multiplayer-chat`'s mirror: `subscribe` for continuous updates,
 * plus `participantId`/`get` to read this client's own slot and `update` to write the stroke array
 * back into it without clobbering other keys (`push`, declared here for that contrast, REPLACES the
 * whole slot; see `draw-core.ts`). `wait` is not declared: this plugin has no barrier, only a
 * continuously-open trial ended by duration/button/condition, so no cancelled `wait()` can reach it.
 * What the cancellation contract does mean here is that jsPsych tears the subscription down at
 * abort/experiment end, so the trial's own stroke-push timer has to die with it — which is why it is
...
export interface MultiplayerApiLike {
   ...
   * which is exactly what lets this plugin push a live stroke every `push_interval_ms`.
